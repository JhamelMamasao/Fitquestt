...
import { StyleSheet, View, Pressable, TextInput, Text, Image, ScrollView, TouchableOpacity} from 'react-native';
...
const FinalDashboard = () => {
  ...
  const data = {
    data: [totalKm / 100] // Assuming totalKm is a percentage value out of 100
  };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        ...
        // Sum the chart values
        const totalKm = chartValues.reduce((acc, value) => acc + value, 0);

        setChartData(Object.entries(data).map(([date, total]) => ({ date, total })));
        setChartLabels(chartLabels);
        setChartValues(chartValues);
        setTotalKm(totalKm.toFixed(2)); // Assuming you want to format it to 2 decimal places
  ...
  return (
    <SafeAreaView style={styles.container} onLayout={onLayoutRootView}>
      ...
      <View style={styles.TextContainer}>
        ...
        <Text style={styles.TotalKm}>{totalKm} km</Text>
