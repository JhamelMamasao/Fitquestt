...
const TaskWalkingScreen = () => {
    ...
    const Quest = useCallback(async () => {
        try {
            ...
            console.log('Quests:', results); // Log the quests array

            const walkingTask = results.find(quest => quest.difficulty === difficulty && quest.type === 'Walking');

            if (walkingTask) {
                setDescription(walkingTask.description);
                setDistance(walkingTask.distance);
                setRewards(walkingTask.prize);
            }
