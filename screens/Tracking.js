...
function Tracking() {
  ...
  useEffect(() => {
    const getLocationPermissionAndWatchPosition = async () => {
      try {
        ...
        await Location.watchPositionAsync({ accuracy: Location.Accuracy.High, timeInterval: 1000, distanceInterval: 10 }, locationCallback);
      } catch (error) {
  ...
  const locationCallback = async (locationData) => {
    ...
    if (runStarted) {
      ...
      if (runCoordinates.length > 1) {
        ...
        try {
          await updateAPIWithLocation(lastCoordinate.latitude, lastCoordinate.longitude, "string");
        } catch (error) {
  ...
  const startRun = async () => {
    ...
    // Call the API with the initial location
    try {
      await updateAPIWithLocation(coords.latitude, coords.longitude, taskSlug);
    } catch (error) {
      console.error("Error updating API:", error);
    }
  ...
  const stopRun = () => {
    setRunStarted(false);
    setRunDistance(0); // Reset the run distance
    setRunCoordinates([]); // Reset the run coordinates
  };
  ...
  const quest = taskSlug;
  const [latitude, setLatitude] = useState(null);
  const [longitude, setLongitude] = useState(null);

  useEffect(() => {
    updateAPIWithLocation(latitude, longitude, quest);
  }, [latitude, longitude, quest]);
  ...
  const updateAPIWithLocation = async (latitude, longitude, quest) => {
    try {
      ...
      const response = await fetch('https://fit-quest.azurewebsites.net/api/location/create', {
        method: 'POST',
        headers: headers,
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update location: ${responseData.message || 'Unknown error'}`);
      }
      ...
      // Check if the response contains the expected data
if (Math.abs(responseData.latitude - latitude) < 0.0001 && Math.abs(responseData.longitude - longitude) < 0.0001 && responseData.quest === quest) {
  console.log('Latitude, longitude, and quest successfully pushed to the API.');
} else {
  console.warn('API response does not match the sent data.');
}
