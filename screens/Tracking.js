  ...
  const locationCallback = async (locationData) => {
    ...
    if (runStarted) {
      ...
      if (runCoordinates.length > 1) {
        ...
        // Call the API with the new location
        try {
          await updateAPIWithLocation(lastCoordinate.latitude, lastCoordinate.longitude, taskSlug);
        } catch (error) {
...
const updateAPIWithLocation = async (latitude, longitude, quest) => {
  try {
    ...
    if (responseData.latitude === latitude && responseData.longitude === longitude && responseData.quest === quest) {
